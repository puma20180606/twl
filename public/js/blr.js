...
function htmlspecial_decode(data){
	data=data.replace(/&amp;/g,'&')
	data=data.replace(/&lt;/g,'<')
	data=data.replace(/&gt;/g,'>')
	return data;
...
function showlog(data,weekdate){	
	...
	for(var i in data){
		...
		offset=td.offset();
		var log=new Worklog({'color':'#006699','id':'log'+row+'_'+j,top:offset.top,left:offset.left,width:td.width(),height:td.height()},data[i].date,fromtime,totime,htmlspecial(data[i].content),null,data[i].label)
		if(data[i].archive==1){
			log.setArchive()
		}
		log.init()
